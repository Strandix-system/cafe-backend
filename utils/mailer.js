...
export default {
  ...
  mailSend: async (emailRequest) => {
    ...
    if (emailRequest.template === CONSTANTS.DRIVERS.SIGNUP) {
      ...
      emailRequest.template === CONSTANTS.ADMIN.RESCHEDULEDNOTACCEPTED
    ) {
      subject = 'Rescheduled Ride Not Accepted!';
      emailTemplate = './templates/Ride-Not-accepted-ReScheduledRide.html';
    } else if (
      emailRequest.template === CONSTANTS.RIDERS.NOTACCEPTEDSCHEDULERIDE
    ) {
      subject = 'Scheduled Ride Not Accepted!';
      emailTemplate = './templates/Ride-Not-accepted-ScheduledRide.html';
    } else if (
      emailRequest.template === CONSTANTS.RIDES.RIDESCHEDULED60MINUTES
    ) {
      //Send an email to driver before 60 minutes of scheduled ride
      subject = 'Scheduled Ride Reminder!';
      emailTemplate = './templates/Schedule-ride-reminder-60min.html';
    } else if (
      emailRequest.template === CONSTANTS.RIDES.DRIVERCHANGED_NOW_RIDER
    ) {
      subject = 'Ride!';
      emailTemplate = './templates/RideNow-driver-changed-rider.html';
    } else if (
      emailRequest.template === CONSTANTS.RIDES.DRIVERCHANGED_NOW_DRIVER
    ) {
      subject = 'Ride!';
      emailTemplate = './templates/RideNow-driver-changed-driver.html';
    } else if (
      emailRequest.template === CONSTANTS.RIDES.DRIVERSCHEDULE_30_MINUTES_BEFORE
    ) {
      //send an email to driver before 30 minutes of scheduled ride
      subject = 'Scheduled Ride Reminder!';
    ...
    if (emailTemplate) {
      ...
      mailMessage = mailMessage.replace(
        /{RiderName}/g,
        emailRequest?.riderName,
      );
      mailMessage = mailMessage.replace(
        /{DriverName}/g,
        emailRequest?.driverName,
      );
      mailMessage = mailMessage.replace(
        /{pickupAddress}/g,
        emailRequest?.pickupAddress,
      );
      mailMessage = mailMessage.replace(
        /{dropOffAddress}/g,
        emailRequest?.dropOffAddress,
      );
      mailMessage = mailMessage.replace(/{fare}/g, emailRequest?.fare);
      mailMessage = mailMessage.replace(
        /{insurance}/g,
        emailRequest?.insurance,
      );
      mailMessage = mailMessage.replace(
        /{reservedDateTime}/g,
        emailRequest?.reservedDateTime,
      );
      mailMessage = mailMessage.replace(
        /{minutes}/g,
        emailRequest?.arrivedTimeText,
      );
      mailMessage = mailMessage.replace(
        /{timeRemaining}/g,
        emailRequest?.timeRemaining,
      );
      mailMessage = mailMessage.replace(/{message}/g, emailRequest?.message);
      mailMessage = mailMessage.replace(/{tip}/g, emailRequest?.tip);
      mailMessage = mailMessage.replace(
        /{TNSCharge}/g,
        emailRequest?.TNSCharge,
      );
